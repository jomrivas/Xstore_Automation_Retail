*** Settings ***
Documentation     Script de venta con forma de pago Tarjeta Externa no Integrada.
...
*** Test Cases ***
Flujo de venta CCF con items precio regular y forma de pago Tarjeta Externa
    [Documentation]    Flujo de venta con documento CCF, items a precio regular y forma de pago Tarjeta Externa (Integración SERFINSA).
...
Accion_SeleccionarFormaDePago
    # Seleccionar forma de pago tarjeta externa y confimración de monto
    ${num_autorizador}=    Generate Random String    5    [NUMBERS]

    Send And Wait    3
    Send And Wait    1
    Send And Wait    {ENTER}
    Send And Wait    {ENTER}
    Send And Wait    ${num_autorizador}
    Sleep    1s
    Activar Ventana    ${VENTANA_PRINCIPAL}
    Send And Wait    {ENTER}
...
Accion_GuardarImpresion
    # Al no estar disponible a impresión de Ticket se utilizara este metodo
    ...
    ELSE
        Log    No se pudo activar la ventana.  mIntentando con WinWaitActive.

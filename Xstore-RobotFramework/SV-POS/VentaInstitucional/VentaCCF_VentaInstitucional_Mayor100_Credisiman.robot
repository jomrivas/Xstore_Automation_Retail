*** Settings ***
Documentation     Script de venta con forma de pago Credisiman.
...
${CREDISIMAN0}          NONE
${MONTO_IVA_RETENIDO}   NONE


*** Test Cases ***
Flujo de venta Institucional (renteción de IVA) con items precio regular y forma de pago Credisiman
    [Documentation]    Flujo de venta con documento CF Venta Institucional (Rentención de IVA), items a precio regular y forma de pago Credisiman.
    ...
    Accion_SeleccionarFormaDePago_IngresoDatos
...
Inicializar Entorno
    ...
    Set Suite Variable    ${CREDISIMAN0}          ${env["CREDISIMAN0"]}
    Set Suite Variable    ${MONTO_IVA_RETENIDO}   ${env["MONTO_IVA_RETENIDO"]}
...
Accion_MenuPrincipal_TiposComprobantes
    ...
    Send And Wait    {F1}
    Activar Ventana    ${VENTANA_PRINCIPAL}
    # Seleccionar Venta Institucional
    Send And Wait    {F4}
...
Accion_SeleccionarFormaDePago_IngresoDatos
    # Crea un número ficticio de certificado de 9 caracteres aleatoreos
    ${num_certificado}=    Generate Random String    9    [NUMBERS]
    # Forma de Pago
    Send And Wait    {F10}
    ...
    Send And Wait    {ENTER}
    Send And Wait    {F8}
    # Selecionar "Otros medios de pago"
    Send And Wait    {a}
    # Seleccionar "Retención de IVA"
    Send And Wait    {2}
    # Ingreso de Número de Certificado y nombre
    Send And Wait    ${num_certificado}
    Send And Wait    {ENTER}
    Send And Wait    Pruebas Automatizacion - Powered by Jomrivas
    Sleep    0.5s
    Send And Wait    {F8}
    Sleep    0.5s
    Send And Wait    {ENTER}
...
Accion_SeleccionarFormaDePago
    Send And Wait    {1}

Accion_IngresarSerie_MontoVenta
    # Serie - Número de tarjeta
    Activar Ventana    ${VENTANA_PRINCIPAL}
    Sleep    1s
    # Retraso en ingreso de serie
    Auto It Set Option    SendKeyDelay    70
    @{digitos}=    Split String To Characters    ${CREDISIMAN0}
    
    FOR    ${char}    IN    @{digitos}
        Send    ${char}
        # Pausa de 100ms entre cada número.
        Sleep    0.1s
    END
    Sleep    0.5s
    Send And Wait    {ENTER} 
    # Ingreso monto de la venta
    Send And Wait    ${MONTO_IVA_RETENIDO}
    Send And Wait    {ENTER}
    Sleep    0.5s
    Send And Wait    {ENTER}
    
Accion_ConfirmaDocumentoEmitidoEnLinea
    # Confirmación FEL - INFILE
    Activar Ventana    ${VENTANA_PRINCIPAL}
    Sleep    2s
    Send And Wait    {F8}
    Sleep    1s
